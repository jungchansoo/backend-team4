...
<body>
	...
	<form action="/join" method="post">
		...
		<br> <label for="userName">이름:</label> <input type="text"
...
$("#id-check-btn").click(function() {
    ...
    if (!userId) {
        $("#id-check-result").text("아이디를 입력해주세요.").css("color", "red");
        return;
    }
    ...
    $.ajax({
        ...
        success: function(response) {
			...
			const isDuplicate = $('Boolean', response).text()==='true';
            if (isDuplicate) {
        ...
        error: function() {
            $("#id-check-result").text("아이디 확인 중 오류가 발생했습니다.").css("color", "red");
        }
    });
});
